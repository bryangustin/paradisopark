                            ...
                            <div class="hidden h-full lg:flex">
                                ...
                                <PopoverGroup class="px-4 bottom-0 inset-x-0">
                                    <div class="h-full flex justify-center space-x-8">
                                        ...
                                        <!-- Other btn -->
                                        <!-- :class="{change_color_text: scrollPosition > 20}" -->
                                        <router-link v-for="firstpage in navigation.firstpages" :key="firstpage.name" :to="firstpage.href" class="transition_navbar flex items-center font-light text-black hover:text-grey-400">{{ $t(firstpage.name) }}</router-link>
                                        ...
                                        <Popover v-for="category in navigation.categories" :key="category.name" class="flex">
                                            <div class="services_menu relative flex">
                                                ...
                                                <div class="PopoverPanel absolute top-full font-light text-gray-500">
                                                    <!-- Presentational element used to render the bottom shadow, if we put the shadow on the actual panel it pokes out the top, so we use this shorter element to hide the top of the shadow -->
                                                    <div class="absolute inset-0 top-1/2" aria-hidden="true" />
...
<footer class=" bg-beige relative bg-white overflow-hidden" aria-labelledby="footer-heading">
    ...
    <div class="relative" id="contact" ref="scrollToMe">
        ...
        <div class="relative max-w-7xl mx-auto lg:grid lg:grid-cols-5">
            <div class="bg-beige pt-8 sm:pt-10 px-4 sm:px-6 lg:col-span-2 lg:px-8 lg:pt-8 xl:pr-12">
                <div class="max-w-lg sm:max-w-full mx-auto">
            ...
            <div class="bg-beige pt-8 px-4 sm:px-6 lg:col-span-3 lg:pt-8 lg:px-8 xl:pl-12">
                <div class="max-w-lg sm:max-w-full mx-auto lg:max-w-none">
                    <form action="#" method="POST" class="grid grid-cols-1 gap-y-6">
                        <div>
                            <label for="full-name" class="sr-only">{{ $t("contact.label.name") }}</label>
                            <input type="text" name="full-name" id="full-name" autocomplete="name" class="block w-full shadow-sm py-2 px-4 bg-white placeholder-gray-500 focus:ring-gray-400 focus:border-gray-400 border-gray-300" :placeholder="$t('contact.placeholder.name')" />
                        </div>
                        <div>
                            <label for="email" class="sr-only">{{ $t("contact.label.mail") }}</label>
                            <input id="email" name="email" type="email" autocomplete="email" class="block w-full shadow-sm py-2 px-4 bg-white placeholder-gray-500 focus:ring-gray-400 focus:border-gray-400 border-gray-300" :placeholder="$t('contact.placeholder.mail')" />
                        </div>
                        <div>
                            <label for="phone" class="sr-only">{{ $t("contact.label.phone") }}</label>
                            <input type="text" name="phone" id="phone" autocomplete="tel" class="block w-full shadow-sm py-2 px-4 bg-white placeholder-gray-500 focus:ring-gray-400 focus:border-gray-400 border-gray-300" :placeholder="$t('contact.placeholder.phone')" />
                        </div>
                        <div>
                            <label for="message" class="sr-only">{{ $t("contact.label.message") }}</label>
                            <textarea id="message" name="message" rows="4" class="block w-full shadow-sm py-2 px-4 bg-white placeholder-gray-500 focus:ring-gray-400 focus:border-gray-400 border border-gray-300" :placeholder="$t('contact.placeholder.message')" />
                            </div>
              <div class="flex justify-end">
                <button type="submit" class="inline-flex justify-center py-2 px-6 shadow-sm text-base font-medium border bg-white text-black hover:text-white hover:bg-black focus:ring-0 focus:outline-none">{{$t("contact.submit")}}</button>
              </div>
            </form>
          </div>
        </div>
...
/* 8/23/2022 ss */
.services_menu:hover .PopoverPanel {
    display: block;
}
